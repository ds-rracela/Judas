...
class TrainNSVM():
    ...
    def __init__(self, X, y, Number_trials):
        score_train = []
        score_test = []
    ...
    def result(self):
        return ['Nonlinear SVM (RBF)', '{:.2%}'.format(np.amax(self.score)), \
                'gamma = {0}'.format(self.CC[np.argmax(self.score)]), self.top_predictor]
...
class TrainNSVMPoly():
    ...
    def result(self):
        return ['Nonlinear SVM (Poly)', '{:.2%}'.format(np.amax(self.score)), \
                'C = {0}'.format(self.CC[np.argmax(self.score)]), self.top_predictor]